 ...
 * 
 * @deprecated This class is being replaced by {@link LuceneBatchInserterIndexProviderNewImpl} as of Neo4j 1.7.
 */
public class LuceneBatchInserterIndexProviderImpl implements BatchInserterIndexProvider {
	...
	private Map<String, String> config(Class<? extends PropertyContainer> cls, String indexName, Map<String, String> config) {
		...
		if (config != null) {
			config = MapUtil.stringMap(new HashMap<String, String>(config), IndexManager.PROVIDER, LuceneIndexImplementation.SERVICE_NAME);
	...
	private BatchInserterIndex index(IndexIdentifier identifier, Map<String, String> config) {
		...
		if (index == null) {
			index = new LuceneBatchInserterIndex(new File(inserter.getStore()), identifier, config);
